...
class TurboWeather {
  ...
  async fetch() {
    var url = "";
    if (metric) {
      url = "https://api.open-meteo.com/v1/forecast?latitude=" + lat + "&longitude=" + lon + "&current=temperature_2m,is_day,weather_code,wind_speed_10m,wind_direction_10m&hourly=temperature_2m,relative_humidity_2m,precipitation,weather_code,wind_speed_10m,wind_direction_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max&timezone=auto";
    } else {
      url = "https://api.open-meteo.com/v1/forecast?latitude=" + lat + "&longitude=" + lon + "&current=temperature_2m,is_day,weather_code,wind_speed_10m,wind_direction_10m&hourly=temperature_2m,relative_humidity_2m,precipitation,weather_code,wind_speed_10m,wind_direction_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max&timezone=auto";
    }
  ...
  getkeylength(args) {
      const data = args.JSON == 'weather data' ? resultjson : locationdata;

      if (data && args.KEY in data && data[args.KEY] && data[args.KEY].length) {
          return data[args.KEY].length;
      } else {
          return "undefined"; // or handle the case when the property or array is undefined
      }
  }
